...
func (is incidentService) ShowIncident(evt *socketmode.Event, incidentID string) {
	...
	startedAt := time.Unix(getIncident.CreatedAt, 0).Format("Monday, Jan 2, 2006 at 3:04 PM")
	...
	if getIncident.Status == incident.Resolved {
		completedAt := time.Unix(*getIncident.UpdatedAt, 0).Format("Monday, Jan 2, 2006 at 3:04 PM")
